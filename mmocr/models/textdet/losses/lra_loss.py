...
class LRALoss(nn.Module):
    ...
    def ohem(self, predict, target, train_mask):
        ...
        if n_pos.item() > 0:
            ...
            print(type(predict[neg]),type(target[neg]))
            # print("第一个：",sum((predict[neg] < 0) | (predict[neg] > 1)),predict[neg].size())
            # print("第二个：",sum((target[neg] < 0) | (target[neg]> 1)),target[neg].size())
            #print(predict[neg], target[neg])
