...
class LRAHead(HeadMixin, BaseModule):
    ...
    def get_boundary(self, score_maps, img_metas, rescale):
        ...
        assert len(score_maps) == len(self.scales)#8,16,32
        # print(img_metas)
        # haha
        boundaries = []
        ...
        #print(len(boundaries),len(boundaries[0]),boundaries)#187个  187+0+0
        #print("************************************************")
        
        boundaries, _ = poly_nms(boundaries, self.nms_thr, with_index=True)
        #print(len(boundaries),len(boundaries[0]),boundaries)#琛 抑制为20个
