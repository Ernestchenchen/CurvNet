...
def eval_hmean_e2e(eval_dataset, results, coco_annos, logger=None):
    ...
    #print(img_ids)
    # with open('/mnt/data/experiments/datasets/spinal-AI2024/spinal_AI2024_all_stage4_discard.txt', 'r') as file:  
    #         # 使用列表推导式读取每一行并转换为整数，然后存储到列表中  
    #        discard = [int(line.strip()) for line in file]  
    
    output = []
    #print(len(results))
    
    for i, img_id in enumerate(img_ids):
        # if img_id in discard:
        #     res = results[0]
        # else:
        #     res = results[i]
        ...
        for j in range(len(ins)):
            ...
            out = {
                ...
                "image_id": int(img_name[0:4]) if eval_dataset == 'ctw1500' else i,
                # "image_id": i,
                "category_id": 1, #if img_id not in discard else 0,
                "polys": pts,
                "rec": rec,
                "score": score
            }
    ...
    if eval_dataset == 'ctw1500':
        ...
        with open('mmocr/core/evaluation/evaluation_e2e/custom_res/my_text_results.json', 'w') as f:
            json.dump(output, f)
        dataset_name = ['custom']
        outdir = 'mmocr/core/evaluation/evaluation_e2e/custom_res'
    #haha
    cfg = {}
    best_det_hmean = -1
    best_e2e_hmean = -1
    ...
    #for t in [0.1, 0.11, 0.12, 0.13, 0.14, 0.15, 0.16, 0.17, 0.18, 0.19, 0.20,]:# 0.21, 0.22, 0.23, 0.24]:
    for t in [0.1, 0.12, 0.14, 0.16, 0.18, 0.2, 0.22, 0.23, 0.24, 0.26, 0.28, 0.3, 0.35, 0.4, 0.5]:
        ...
        res = e.evaluate()#OrderedDict([('E2E_RESULTS', {'precision': 0.0, 'recall': 0.0, 'hmean': 0.0}), ('DETECTION_ONLY_RESULTS', {'precision': 0.0, 'recall': 0.0, 'hmean': 0.0})])
