...
def validate_data(gtFilePath, submFilePath, evaluationParams):
    ...
    """
    print(gtFilePath,submFilePath)
    print(evaluationParams['GT_SAMPLE_NAME_2_ID'],evaluationParams['DET_SAMPLE_NAME_2_ID'],evaluationParams['CRLF'],evaluationParams['LTRB'],evaluationParams['CONFIDENCES'])
    
    gt = rrc_evaluation_funcs.load_zip_file(gtFilePath, evaluationParams['GT_SAMPLE_NAME_2_ID'])#琛 正则表达式 001.txt-904.txt
    ...
    #琛 返回格式：points,confidence,transcription 举例：[78.0, 26.0, 108.0, 21.0, 86.0, 38.0, 114.0, 30.0] 0.0 True_dou
    #Validate format of results
    for i in subm:
        #print(gt)
        
        k=chen_add_and_format(i)
        # haha
        if (k in gt) == False :  
            raise Exception("The sample %s not present in GT" %k)
       
        chenchen=rrc_evaluation_funcs.validate_lines_in_file(k,subm[i],evaluationParams['CRLF'],evaluationParams['LTRB'],True,evaluationParams['CONFIDENCES'])
        # print(chenchen)
        # haha
    
    
def chen_add_and_format(k_str):  
    # 将字符串转换为整数  
    k = int(k_str)  
    ...
    if result >= 1000000:  
        ...
        # 如果结果已经是六位或更少位，则直接转换为字符串（可能需要前导零）  
        result_str = str(result).zfill(6)  # zfill确保至少六位，并在需要时添加前导零  
