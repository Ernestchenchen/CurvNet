...
def calculate_sum(point):
    return point[0] + point[1]

def calculate_distance(point1, point2):
    return math.sqrt((point2[0] - point1[0])**2 + (point2[1] - point1[1])**2)

def sort_and_connect_points(pts):
    # Step 1: Calculate sum for each point
    pts_sorted = sorted(pts, key=calculate_sum)

    # Step 2: Connect the points
    connected_pts = []
    connected_pts.append(pts_sorted[0])  # Starting point

    while len(connected_pts) < 4:
        last_point = connected_pts[-1]
        closest_point = None
        min_distance = float('inf')

        for point in pts_sorted:
            if point not in connected_pts:
                distance = calculate_distance(last_point, point)
                if distance < min_distance:
                    closest_point = point
                    min_distance = distance

        connected_pts.append(closest_point)

    # Step 3: Ensure clockwise order
    if (connected_pts[1][0] - connected_pts[0][0]) * (connected_pts[2][1] - connected_pts[1][1]) - (connected_pts[1][1] - connected_pts[0][1]) * (connected_pts[2][0] - connected_pts[1][0]) > 0:
        connected_pts = connected_pts[::-1]
...
def get_tl_line_values_gt(line,LTRB=True,withTranscription=False,withConfidence=False,imWidth=0,imHeight=0):
    ...
    # print(points,confidence,transcription)
    # haha
...
def validate_clockwise_points(points):
    ...
    # 计算凸包
    #print("1",pts)
    #pts = sort_and_connect_points(pts)
    #pts = sorted(pts, key=lambda pt: (pt[1], pt[0]))
    #print("1.5",pts)
    #t=pts[2]
    #pts[2]=pts[3]
    #pts[3]=t
    ...
    # # 按顺序连接顶点
    # pts = [points[i] for i in vertices]
...
def main_evaluation(p,det_file, gt_file, default_evaluation_params_fn,validate_data_fn,evaluate_method_fn,show_result=True,per_sample=True):
    ...
    p['s'] =det_file #'det.zip'#{'g': 'mmocr/core/evaluatio...custom.zip', 's': 'det.zip'}
