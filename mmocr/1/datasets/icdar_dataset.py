...
class IcdarDataset(CocoDataset):
    ...
    def __init__(self,
        ...
        self.chen_filename = '/mnt/data/experiments/datasets/spinal-AI2024/spinal_AI2024_all_stage6_discard.txt'  

  
        # 打开文件并读取所有行  
        with open(self.chen_filename, 'r') as file:  
            # 使用列表推导式读取每一行并转换为整数，然后存储到列表中  
            self.discard = [int(line.strip()) for line in file]  
